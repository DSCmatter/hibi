...
Edit English copy in its source file, then run `npm run copy:catalog`. Run `npm run copy:check` to check that the catalog is current. Keep placeholders such as `{name}` in translations; the app supplies their values.
