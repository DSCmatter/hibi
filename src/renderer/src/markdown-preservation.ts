...
export function needsSourceEditing(source: string): boolean {
  if (readFrontmatter(source)) return true
  const hasDefinition = source.includes(']:')
  if (!source.includes('<') && !hasDefinition) return false
  if (hasDefinition && /^\s{0,3}\[[^\]]+\]:/m.test(source)) return true
