...
The sidebar contains a square graph preview. Expand opens a fullscreen dialog; the sidebar simulation pauses until it closes. **Connections** lists notes linked to or from the current note, independent of the graph filter. Click a node or connection to open its note with the normal unsaved-change checks. Drag nodes to arrange them; drag the background to pan; scroll or use the buttons to zoom and fit. Keyboard users can tab to nodes and press enter/space, or use arrow keys on the graph background to pan. Filtering narrows filenames/paths. Workspace changes, active drafts, and window focus refresh the graph automatically.
