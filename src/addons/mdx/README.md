...
Turn on **MDX** in **Settings → Addons** to edit `.mdx` files. Use source view for highlighted source and formatting tools, or side-by-side view for a preview. Normal view is unavailable for this format.

Hibi includes the parser, so the preview needs no separate tool. Use **Settings → Formats → MDX** to check tools and open the plugin's settings.
