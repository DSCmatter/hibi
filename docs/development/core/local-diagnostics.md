...
Admission happens before stack projection. Native error branding and own data descriptors allow already materialized original stacks without invoking getters or `Error.prepareStackTrace`. The complete header is removed using bounded inert name/message descriptors so multiline error text cannot impersonate frames. Unknown headers or prototypes, lazy stack accessors and arbitrary thrown objects are omitted. Neither names nor messages enter the result. Browser-owned error coordinates can provide an original frame. Stack text is bounded before parsing, and only exact trusted artifact locations survive. Eval frames, unknown sourceURL labels, query-bearing URLs and unknown locations are omitted. The catalog constrains output; it cannot authenticate a deliberately forged stack from already compromised application code. A caller-created stack is not evidence of the original failure and must never be substituted.
