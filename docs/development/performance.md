...
Built-in code languages retain metadata at startup, cache in-flight and completed parser loads, and update decorations without changing source or undo history. Disabled languages stay disabled even if an import finishes afterward. Async Markdown exports await requested parsers; synchronous API v1 exports may initially contain escaped plain code. Settings and history UI load on interaction. The small command palette stays in the shell bundle: a lazy fallback in the app's flex layout resized the editor before opening the modal, causing first-open stutter. Palette search still mounts settings discovery on demand instead of duplicating control metadata.

Run `node scripts/benchmark-transitions.mjs` after building to compare first and repeated palette opens across three fresh profiles. `HIBI_BENCH_RUNS` changes the sample count; an optional checkout path measures another built revision with the same sampler. The report records first-visible-frame latency and animation-frame gaps for 800 ms around each open. These local hidden-window measurements include automation and host scheduling; use them to compare revisions on the same machine, not as a cross-platform frame-rate guarantee. The startup regression test separately checks that first open never inserts a loading screen into the app layout or resizes the editor, while still discovering plugin settings.
